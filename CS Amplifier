...
3.AC Analysis:\
![Image](https://github.com/user-attachments/assets/f5b1b4cb-c4e3-4c96-ab3b-4589bd699457)\
we got frequency = 210.45GHz\
Gain(dB) = -9.104dB

### Inference:

1. The drain current increases in direct proportion to the MOSFET’s width when all other parameters remain constant.\
2. AC analysis establishes the MOSFET’s operating point and ensures it stays in the saturation region.\
3. Transient analysis evaluates the MOSFET’s response to a time-varying AC input, such as a sine wave.\
4. The circuit produces an amplified output, exhibiting a 180-degree phase shift between the input and output signals.\
5. AC analysis is essential for determining the circuit’s gain and frequency response characteristics.
